...
git pull --rebase --autostash -q origin staging   # 다른 채팅의 미커밋 변경은 autostash로 보존
...
DIRTY=$(git status --porcelain | grep -v 'tools/system-designer.html' || true)
PROMOTED=0
if [ -z "$OTHERS" ] && [ -z "$DIRTY" ]; then
  "$HOME/Projects/AudioAZ/promote-to-live.sh" >/dev/null && { echo "  staging→main 승격 완료"; PROMOTED=1; }
else
  echo "  ⚠️ staging에 다른 채팅의 변경(커밋:${OTHERS:+있음}${OTHERS:-없음} · 미커밋:${DIRTY:+있음}${DIRTY:-없음}) — 라이브 승격 보류, staging까지만 반영"
fi
...
for i in $(seq 1 30); do
  ...
  if [ "$PROMOTED" = "1" ]; then
    ...
    [ "$A" -ge 1 ] && { echo "✅ github.io $VER 라이브 (홈페이지는 staging 대기 — 승격 보류됨)"; exit 0; }
  fi
  sleep 10
done
